...
class BuilderBase:
    def __init__(self, cmd, mode, prefixPath=None, cstring=None):
        ...
        # Check if cstring is set
        if cstring is None:
            errorMsg = 'No compiler set. Please set environment variable LLVM_COMPILER'
...
class ClangBuilder(BuilderBase):
    ...
    def getCompiler(self):
        if self.mode == "wllvm++":
            env, prog = 'LLVM_CXX_NAME', self.cstring + '++'
        elif self.mode == "wllvm":
            env, prog = 'LLVM_CC_NAME', self.cstring
...
def getBuilder(cmd, mode):
    ...
    if "clang" in cstring:
        return ClangBuilder(cmd, mode, pathPrefix, cstring)
